...
实际节省需用相同任务做对照，记录端到端 input/output token、工具动作、耗时和任务成功率。较长版 Skill 与无 Skill 对照没有稳定收益，因此我们换成了 921 字符的精简版。在 Zed 配置这一种跨文件开发任务上，反序配对分别少用 22.9% 和 5.2% token，合计少用 15.2%；工具动作少 1 次，耗时少 4.1%。目前只有一个任务类型，不能据此推断普遍效果，2x 目标尚未达到。完整数据、限制和后续研究见[调研依据和限制](efficiency-research.md)。
