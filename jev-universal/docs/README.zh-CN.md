...
实际节省需用相同任务做对照，记录端到端 input/output token、工具动作、耗时和任务成功率。较长版 Skill 与无 Skill 对照没有稳定收益，因此换成了 921 字符精简版。Zed 配置开发任务中，它两次分别少用 22.9% 和 5.2% token，合计少 15.2%；但第二种开发任务（给输出召回 CLI 增加搜索上下文）两次合计多用 4.8%，工具动作多 3 次，耗时多 0.9%。结果混合且任务类型有限，不能据此推断普遍效果，2x 目标尚未达到。完整数据、限制和后续研究见[调研依据和限制](efficiency-research.md)。
