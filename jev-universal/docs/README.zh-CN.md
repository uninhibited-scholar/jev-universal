...
实际节省需用相同任务做对照，记录端到端输入/输出 token、工具调用、耗时和任务成功率。最近四组成对评测结果仍然混合：一轮少用 26.7% token，一轮多用 9.7%，同类任务反向顺序复测少用 14.6%，Zed 配置功能开发又多用 2.5%。同类响应校验任务的两次测试合并后仅少用 2.5%。共同测试门禁都通过，但 Zed 任务中的 baseline 删除了原有中文客户端配置指引。当前证据不能证明稳定节省，更未达到 2 倍目标。完整数据、限制和后续目标见[调研依据和限制](efficiency-research.md)。
