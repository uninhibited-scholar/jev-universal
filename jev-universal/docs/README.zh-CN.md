...
实际节省需用相同任务做对照，记录端到端输入/输出 token、工具调用、耗时和任务成功率。五组成对评测结果仍然混合：一轮少用 26.7% token，一轮多用 9.7%，同类任务反向顺序复测少用 14.6%；Zed 配置功能两次反向顺序测试分别多用 2.5% 和 7.7%，合并后多用 4.7%。最近一次 Skill 修改还删弱了英文指南中保留现有客户端设置的提醒，未通过文档无回归门槛。共同测试通过，但当前证据不能证明稳定节省，更未达到 2 倍目标。完整数据、限制和后续目标见[调研依据和限制](efficiency-research.md)。
