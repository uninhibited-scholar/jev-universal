...
实际节省需用相同任务做对照，记录端到端输入/输出 token、工具调用、耗时和任务成功率。最近两版精简候选在同一 Zed 配置任务中分别多用 39.0%、43.8% token；上一轮跨层 bug 修复也多用 0.7%，但动作少 10.7%、耗时少 6.5%。测试过的版本都没有证明稳定省 token，早期正向结果还存在行为回归、未加载或过晚加载等问题。当前证据更未达到最低 2 倍目标（10 倍为挑战目标）。完整数据、限制和后续目标见[调研依据和限制](efficiency-research.md)。
