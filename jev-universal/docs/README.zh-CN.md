...
实际节省需用相同任务做对照，记录端到端输入/输出 token、工具调用、耗时和任务成功率。同一版 Skill 在 key-file 配置任务的反序配对分别少用 21.8%、46.1% token；但在全 pinned 快速路径修复任务的反序配对分别多用 28.8%、10.1%。四轮两类任务的测试和目标 Ruff 均通过，但证据尚未证明跨任务稳定节省，更未达到至少减半的目标（10 倍为挑战目标）。完整数据、限制和后续目标见[调研依据和限制](efficiency-research.md)。
