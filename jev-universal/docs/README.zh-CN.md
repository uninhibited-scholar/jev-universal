...
实际节省需用相同任务做对照，记录端到端输入/输出 token、工具调用、耗时和任务成功率。六组成对评测结果仍然混合：一轮少用 26.7% token，一轮多用 9.7%，同类任务复测少用 14.6%；Zed 配置功能三次测试分别多用 2.5%、7.7%、3.8%。最新版本保住了相邻文档指引，但多用 3.8% token、耗时多 7.0%，目前只有一次修复后验证。共享测试门禁通过，但证据仍不能证明稳定节省，更未达到最低 2 倍目标（10 倍为挑战目标）。完整数据、限制和后续目标见[调研依据和限制](efficiency-research.md)。
